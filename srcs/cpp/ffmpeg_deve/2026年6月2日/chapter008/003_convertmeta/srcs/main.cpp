...
void get_zh( char *key, char *src_str, char *dst_str, size_t dst_len ) {
	...
	size_t tmp_strLen = src_len + 1;
	char *tmp_str = new char[ tmp_strLen ];
...
#endif
	printf( "metadata %s=%s, length=%lld, is_iso=%s, is_gbk=%s, is_utf8=%s\n",
...
int main( int argc, char **argv ) {
	...
	while( ( tag = av_dict_get( fmt_ctx->metadata, "", tag, AV_DICT_IGNORE_SUFFIX ) ) ) {
		...
		int len = strlen( tag->value ) * 3 / 2 + 1;
		char *dst_str = new char[ len ];
		// 获取可在操作系统命令行正常显示的中文文本
		get_zh( tag->key, tag->value, dst_str, len );
