...
int init_video_filter( const char *filters_desc ) {
	...
	auto avGetPixFmtName = av_get_pix_fmt_name( AV_PIX_FMT_YUV420P );
...
int init_audio_filter( const char *filters_desc ) {
	...
	char args[ 1024 ]; // 临时字符串，存放输入源的媒体参数信息，比如音频的采样率、采样格式等
	auto sampleFmtName = av_get_sample_fmt_name( audio_decode_ctx->sample_fmt );
	snprintf( args, sizeof( args ),
			"sample_rate=%d:sample_fmt=%s:channel_layout=%s:channels=%d:time_base=%d/%d",
			audio_decode_ctx->sample_rate, sampleFmtName,
	...
	AVDictionary *option = nullptr;
	auto outSampleFmts = AV_SAMPLE_FMT_FLTP;
	auto avGetSampleFmtName = av_get_sample_fmt_name( outSampleFmts );
	ret = av_dict_set( &option, "sample_fmts", avGetSampleFmtName, 0 );
...
int main( int argc, char **argv ) {
	...
	filtersString.append( "[0:v]setpts=expr=0.5*PTS" );
	filtersString.append( ";[0:a]atempo=tempo=2,aformat=sample_fmts=" );
	filtersString.append( avGetSampleFmtName );
	filtersString.append( ",asetnsamples=n=1024:p=0" );
	const char *filters_desc = filtersString.c_str( );
	if( argc > 1 ) {
		src_name = argv[ 1 ];
	}
	if( argc > 2 ) {
	...
	if( init_video_filter( video_filters_desc ) < 0 )
		return -1; // 初始化视频滤镜
	if( init_audio_filter( audio_filters_desc ) < 0 )
		return -1; // 初始化音频滤镜
