...
int init_filter( const char *filters_desc ) {
	...
	ret = av_dict_set( &option, "sample_fmts", av_get_sample_fmt_name( AV_SAMPLE_FMT_NONE ), 0 );
	if( ret < 0 ) {
...
int main( int argc, char **argv ) {
	...
	const char *src_name1 = "2018.mp3";
	const char *dest_name = "output_mixaudio.mp3";
	std::string filters_desc_arg;
	filters_desc_arg.append( "[0:a]aresample=44100,aformat=sample_fmts=fltp,asetnsamples=n=1152[a1]" );
	filters_desc_arg.append( ";[1:a]aresample=44100,aformat=sample_fmts=fltp,volume=1,asetnsamples=n=1152[a2]" );
	filters_desc_arg.append( ";[a1][a2]amix" );
